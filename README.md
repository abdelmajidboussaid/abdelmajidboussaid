# 👋 Hi, I'm Abdelmajid Boussaid

💻 **Freelance .NET Developer**  
🚀 Building reliable, scalable web applications with **ASP.NET Core, EF Core, and Azure**

---

### 🧠 About Me

- 🇨🇦 Based in Québec, Canada  
- 💼 7+ years of experience in **.NET development** (Web API, Blazor, MAUI)  
- ☁️ Skilled in **Azure Cloud**, **CI/CD**, and **Docker**  
- 🧱 Focused on **clean architecture, SOLID principles, and maintainable code**  
- 🎯 Helping teams build efficient and secure cloud-native solutions  

---

### 🛠️ Tech Stack

**Languages & Frameworks**  
C# • .NET 8 • ASP.NET Core • Blazor • MAUI • EF Core  

**Architecture & Patterns**  
Clean Architecture • CQRS • DDD • SOLID Principles  

**Cloud & DevOps**  
Azure • GitHub Actions • Azure DevOps • Docker • Azure Developer CLI (azd)  

**Databases & Tools**  
PostgreSQL • SQL Server • EF Core • LINQ  

**Testing & Quality**  
xUnit • FluentAssertions • Moq  

---

### 📬 Connect with Me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-blue?logo=linkedin&logoColor=white)](https://www.linkedin.com/in/abdelmajidboussaid)
[![GitHub](https://img.shields.io/badge/GitHub-black?logo=github&logoColor=white)](https://github.com/abdelmajidboussaid)
[![Email](https://img.shields.io/badge/Email-D14836?logo=gmail&logoColor=white)](mailto:Abdelmajid.boussaid@gmail.com)


---

### 📊 GitHub Stats

![Majid's GitHub stats](https://github-readme-stats.vercel.app/api?username=abdelmajidboussaid&show_icons=true&theme=transparent)  
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=abdelmajidboussaid&layout=compact&theme=transparent)

---

> 💡 *“Code is like humor. When you have to explain it, it’s bad.” — Cory House*
